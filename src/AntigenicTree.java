...
public class AntigenicTree {
	...
	public static void printParentsList(){
	    try {
                File tFile = new File("out.antigenicTreeList");
                tFile.delete();
                tFile.createNewFile();
                PrintStream stream = new PrintStream(tFile);

		stream.printf("AntigenicType,Parent,Distance\n");
		for (int i = 0; i < nodes.size(); i++){
			int parent = nodes.get(i).getParent();
			double dist = nodes.get(i).getDistance();
			stream.printf("%d,%d,%.4f",i,parent,dist);
			stream.println();
		}
