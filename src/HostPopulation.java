...
public class HostPopulation {
	...
	private HashMap<Integer, Integer> newPhenotypeInfections;
	private HashMap<Integer, Integer> recPhenotypeInfections;
	...
	public HostPopulation(int d) {
		...
		// basic parameters
		newPhenotypeInfections = new HashMap<Integer, Integer>();
                recPhenotypeInfections = new HashMap<Integer, Integer>();
		...
		if (deme == Parameters.initialDeme - 1) {
			...
			for (int i = 0; i < Parameters.initialI; i++) {
                                ...
                                Host h = new Host(v);
				infecteds.add(h);
				IncrementPhenotypeInfections(p);
			}	
	...
	public double getAntigenicDiversity() {
       ...
       public HashMap<Integer, VirusFitnessDist> getViralFitnessDistribution_byAntigenicType() {
        HashMap<Integer, List<Virus>> viruses = new HashMap<Integer, List<Virus>>();
        int infections = getI();
        boolean sampling = false;
        if(infections > Parameters.fitnessDistSamplingMax2){
           sampling = true;
           infections = Parameters.fitnessDistSamplingMax2;
        }
		for (int i = 0; i < infections; i++) {
		              Host h;
                      if(!sampling){
                      ...
                      if (!viruses.containsKey(type)){
                        viruses.put(type,new ArrayList<Virus>());
                      }
		              viruses.get(type).add( v );
		}
		HashMap<Integer, VirusFitnessDist> ret = new HashMap<Integer, VirusFitnessDist>();
		for (Integer type : viruses.keySet()) {
            ret.put(type, getViralFitnessDistribution_( viruses.get(type) ) );
		}
		return ret;
	}
		...
		List<Virus> viruses = new ArrayList<Virus>();
                int infections = getI();
                boolean sampling = false;
        if(infections > Parameters.fitnessDistSamplingMax){
           sampling = true;
           infections = Parameters.fitnessDistSamplingMax;
        }
		for (int i = 0; i < infections; i++) {
                      Host h;
                      if(!sampling){
                      ...
                      viruses.add( h.getInfection() );
		}
		return getViralFitnessDistribution_( viruses );
	}

        public VirusFitnessDist getViralFitnessDistribution_( List<Virus> viruses ) {
	    ...
	    fitDist.N = N;
            ...
            boolean logValues = Parameters.fitnessLogValue;
	...
	private void IncrementPhenotypeInfections( Phenotype mutP ) {
        ...
        if (!newPhenotypeInfections.containsKey(typ)){
			newPhenotypeInfections.put(typ,0);
		}
		newPhenotypeInfections.put(typ, newPhenotypeInfections.get(typ) + 1);
	}

	private void DecrementPhenotypeInfections( Phenotype mutP ) {
		int typ = mutP.antigenicType();
        if (!recPhenotypeInfections.containsKey(typ)){
                        recPhenotypeInfections.put(typ,0);
        }
        recPhenotypeInfections.put(typ, recPhenotypeInfections.get(typ) + 1);
        }
	...
	public void printPhenotypeInfections(int day, PrintStream stream) {
		for (Integer k : newPhenotypeInfections.keySet()) {
		...
		for (Integer k : recPhenotypeInfections.keySet()) {
        ...
        public void printSampleClades(double date, PrintStream stream) {
                if (getI()>0 && Parameters.day >= Parameters.burnin) {
                        // sample viruses
			List<Virus> samps = new ArrayList<Virus>();
			...
			/*
                        ...
                        int samples = Random.nextPoisson(totalSamplingRate);
			*/
			int samples = Parameters.CladeSamples;
			if (getI() < samples/4) samples = getI()/4;
                        for (int i = 0; i < samples; i++) {
                                int index = getRandomI();
                                Host h = infecteds.get(index);
                                Virus v = h.getInfection();
			...
			// 0. initialize
			int mindist = 0;
			List<Virus> clades = new ArrayList<Virus>();
			List<Integer> dists = new ArrayList<Integer>();
			for (Virus v : samps){ 
				clades.add(v);
				dists.add(0);
			}
			// 1. walk back until distance reaches threshold or antigen type changes
			while (mindist < Parameters.thresholdMutationClade){
				mindist = Parameters.thresholdMutationClade;
				for (int i = 0; i < samples; i++){
					Virus v = clades.get(i);
					if (v == null) continue;
					if (v.getParent() == null){
						clades.set(i,null);
						continue;
					}
					if (dists.get(i)<Parameters.thresholdMutationClade && v.getParent().getPhenotype().antigenicType()==v.getPhenotype().antigenicType()){
						clades.set(i,v.getParent());
						dists.set(i, dists.get(i)+v.getPhenotype().ParentalDistance());
					        if (dists.get(i) < mindist) mindist = dists.get(i);
					}
				}
			}
			// 2. correct. Now, we have to check whether any clade is a subclade of any other.
			// TODO

			// 3. summarize
			HashMap<String,List<Virus>> CladeList = new HashMap<String,List<Virus>>();
			for (int i=0; i < samples; i++){
				Virus cv = clades.get(i);
				String k = "null";
				if (cv != null) k = clades.get(i).toString();
				if (CladeList.get(k) == null) CladeList.put(k,new ArrayList<Virus>());
                                CladeList.get(k).add(samps.get(i));
			}
			for ( String k : CladeList.keySet() ){
			        VirusFitnessDist vd = getViralFitnessDistribution_( CladeList.get(k) );
				stream.printf("%.6f,%s,%d,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f", date, k, vd.N, vd.meanBeta, vd.varBeta, vd.meanSigma, vd.varSigma, vd.meanR, vd.varR, vd.covBetaSigma);
				stream.println();
			}
                }
        }

	public void printSampleCladesHeader(PrintStream stream) {
		stream.printf("date,clade,N,meanBeta,varBeta,meanSigma,varSigma,meanR,varR,covBetaSigma");
		stream.println();
	}
